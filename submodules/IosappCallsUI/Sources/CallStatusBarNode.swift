        ...
        if self.didSetupDataForCall !== setupDataForCall {
            ...
            switch content {
                case let .call(sharedContext, account, call):
                    ...
                    let callPeer = IosappEngine(account: account).data.get(IosappEngine.EngineData.Item.Peer.Peer(id: call.peerId))
                ...
                case let .groupCall(sharedContext, account, call):
                    ...
                    if let peerId = call.peerId {
                        callPeerView = IosappEngine(account: account).data.subscribe(IosappEngine.EngineData.Item.Peer.Peer(id: peerId))
                    } else {
                    ...
                    if let groupCall = call as? PresentationGroupCallImpl {
                        let _ = (allowedStoryReactions(engine: IosappEngine(account: account))
