...
//        }
        ...
        callPeerView = IosappEngine(account: self.account).data.subscribe(IosappEngine.EngineData.Item.Peer.Peer(id: self.call.peerId))

        self.peerDisposable = (combineLatest(queue: .mainQueue(),
            IosappEngine(account: self.account).data.subscribe(IosappEngine.EngineData.Item.Peer.Peer(id: self.account.peerId)) |> take(1),
            callPeerView,
