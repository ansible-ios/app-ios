...
final class VideoChatScreenComponent: Component {
    ...
    final class View: UIView, UIGestureRecognizerDelegate {
        ...
        func openTitleEditing() {
            ...
            let _ = (groupCall.accountContext.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: peerId))
        ...
        func update(component: VideoChatScreenComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<ViewControllerComponentContainer.Environment>, transition: ComponentTransition) -> CGSize {
            ...
            if self.appliedCurrentCall != call {
                ...
                case let .group(groupCall):
                    ...
                    let currentAccountPeer = groupCall.accountContext.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: groupCall.accountContext.account.peerId))
                    |> mapToSignal { peer -> Signal<EnginePeer, NoError> in
                        if let peer {
                            return .single(peer)
                        } else {
...
func allowedStoryReactions(engine: IosappEngine) -> Signal<[ReactionItem], NoError> {
    let topReactions = engine.data.subscribe(IosappEngine.EngineData.Item.OrderedLists.ListItems(collectionId: Namespaces.OrderedItemList.CloudTopReactions))
