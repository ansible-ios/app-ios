    ...
    init(context: AccountContext, chatLocation: ChatLocation, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>, subject: ChatControllerSubject?, controllerInteraction: ChatControllerInteraction, chatPresentationInterfaceState: ChatPresentationInterfaceState, automaticMediaDownloadSettings: MediaAutoDownloadSettings, navigationBar: NavigationBar?, statusBar: StatusBar?, backgroundNode: WallpaperBackgroundNode, controller: ChatControllerImpl?) {
        ...
        if case let .messageOptions(_, messageIds, info) = subject {
            ...
            case let .forward(forward):
                let accountPeerSignal = context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: context.account.peerId))
                        ...
                        if hideNames || options.hideCaptions {
                            for media in message.media {
                                ...
                                if let poll = media as? IosappMediaPoll {
                                    ...
                                    updatedMedia.append(IosappMediaPoll(pollId: poll.pollId, publicity: poll.publicity, kind: poll.kind, text: poll.text, textEntities: poll.textEntities, options: poll.options, correctAnswers: poll.correctAnswers, results: IosappMediaPollResults(voters: nil, totalVoters: nil, recentVoters: [], solution: nil, hasUnseenVotes: false, canViewStats: false), isClosed: false, deadlineTimeout: nil, deadlineDate: nil, pollHash: poll.pollHash))
            ...
            case let .reply(reply):
                let replyAccountPeerSignal = context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: context.account.peerId))
                |> mapToSignal { peer -> Signal<EnginePeer, NoError> in
                    if let peer {
                        return .single(peer)
                    } else {
            ...
            case let .link(link):
                ...
                |> mapToSignal { options -> Signal<(ChatControllerSubject.LinkOptions, Peer, Message?, [StoryId: CodableEntry]), NoError> in
                    ...
                    let linkAccountPeerSignal = context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: context.account.peerId))
                    |> mapToSignal { peer -> Signal<EnginePeer, NoError> in
                        if let peer {
                            return .single(peer)
                        } else {
        ...
        self.interactiveEmojisDisposable = (self.context.engine.data.subscribe(IosappEngine.EngineData.Item.Configuration.ApplicationSpecificPreference(key: PreferencesKeys.appConfiguration))
