...
//                params.openPeer(peer, .info)
    ...
    public func makeLinkEditController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)?, text: String, link: String?, apply: @escaping (String?, IosappMediaWebpage?) -> Void) -> ViewController {
        let presentationData = context.sharedContext.currentPresentationData.with { $0 }
