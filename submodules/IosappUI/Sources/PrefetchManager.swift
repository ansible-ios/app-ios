...
private final class PrefetchManagerInnerImpl {
    ...
    init(queue: Queue, sharedContext: SharedAccountContext, account: Account, engine: IosappEngine, fetchManager: FetchManager) {
        ...
        let appConfiguration = engine.data.subscribe(IosappEngine.EngineData.Item.Configuration.ApplicationSpecificPreference(key: PreferencesKeys.appConfiguration))
        |> take(1)
        |> map { view in
