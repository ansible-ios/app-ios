...
private final class DeviceContactDataModernContext: DeviceContactDataContext {
    ...
    init(queue: Queue, accountManager: AccountManager<IosappAccountManagerTypes>, updated: @escaping ([DeviceContactStableId: DeviceContactBasicData]) -> Void, appSpecificReferencesUpdated: @escaping ([EnginePeer.Id: DeviceContactBasicDataWithReference]) -> Void) {
        self.queue = queue
                ...
                let visitor = ChangeVisitor(
                    ...
                    onAdd: { contact in
                        ...
                        for address in contact.urlAddresses {
                            if address.label == "Iosapp", let peerId = parseAppSpecificContactReference(address.value as String) {
