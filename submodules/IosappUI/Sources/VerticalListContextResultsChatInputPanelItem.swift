...
final class VerticalListContextResultsChatInputPanelItem: ListViewItem {
    fileprivate let engine: IosappEngine
    ...
    public init(engine: IosappEngine, theme: PresentationTheme, result: ChatContextResult, resultSelected: @escaping (ChatContextResult, ASDisplayNode, CGRect) -> Bool) {
        self.engine = engine
        self.theme = theme
        self.result = result
