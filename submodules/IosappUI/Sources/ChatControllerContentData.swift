        ...
        init(
            ...
            if case let .peer(peerView) = self.chatLocationInfoData, let peerId = peerId {
                ...
                if let threadId = chatLocation.threadId {
                    threadInfo = context.engine.data.subscribe(IosappEngine.EngineData.Item.Messages.ThreadInfo(peerId: peerId, threadId: threadId))
                ...
                |> mapToSignal { message -> Signal<(message: Message?, threadData: MessageHistoryThreadData?, messageCount: Int), NoError> in
                    ...
                    return context.engine.data.subscribe(
                        IosappEngine.EngineData.Item.Messages.ThreadInfo(peerId: peerId, threadId: replyThreadId),
                        IosappEngine.EngineData.Item.Messages.MessageCount(peerId: peerId, threadId: replyThreadId, tag: MessageTags()),
                        IosappEngine.EngineData.Item.Messages.MessageCount(peerId: peerId, threadId: replyThreadId, tag: MessageTags(), namespace: Namespaces.Message.Local)
                    )
                ...
                if let savedMessagesPeerId {
                    ...
                    savedMessagesPeer = context.engine.data.subscribe(
                        IosappEngine.EngineData.Item.Peer.MainPeer(id: threadPeerId),
                        IosappEngine.EngineData.Item.Peer.Presence(id: threadPeerId),
                        IosappEngine.EngineData.Item.Messages.MessageCount(peerId: peerId, threadId: savedMessagesPeerId.toInt64(), tag: MessageTags()),
                        IosappEngine.EngineData.Item.Messages.ThreadInfo(peerId: peerId, threadId: savedMessagesPeerId.toInt64())
                    )
            ...
            if let peerId = chatLocation.peerId {
                ...
                if let threadId = chatLocation.threadId {
                    ...
                    |> mapToSignal { isForum -> Signal<ChatPresentationInterfaceState.ThreadData?, NoError> in
                        if isForum {
                            return context.engine.data.subscribe(IosappEngine.EngineData.Item.Messages.ThreadInfo(peerId: peerId, threadId: 1))
                            |> map { threadData -> ChatPresentationInterfaceState.ThreadData? in
                                guard let data = threadData else {
                                    return nil
                                }
