...
extension ChatControllerImpl {
                        ...
                        switch navigation {
                            case .info, .default:
                                ...
                                if let messageId = fromMessage?.id {
                                    ...
                                    peerSignal = self.context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: peer.id))
                                }
    ...
    func openBotForumMoreMenu(sourceView: UIView, gesture: ContextGesture?) {
        ...
        if let threadId = self.chatLocation.threadId, let peer = self.presentationInterfaceState.renderedPeer?.chatMainPeer, (peer is IosappChannel || peer is IosappGroup) {
            ...
            if let peer = self.presentationInterfaceState.renderedPeer?.chatMainPeer as? IosappUser, let botInfo = peer.botInfo, botInfo.flags.contains(.forumManagedByUser) {
