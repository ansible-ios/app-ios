    ...
    func navigateToMessage(
        ...
        if isPinnedMessages || forceNew, let messageId = messageLocation.messageId {
            ...
            if let fromIndex = fromIndex {
                    ...
                    if let strongSelf = self {
                        if let index = index {
                            ...
                            if let messageId = messageLocation.messageId {
                                messageSignal = strongSelf.context.engine.data.get(IosappEngine.EngineData.Item.Messages.Message(id: messageId))
                            } else {
                                messageSignal = .single(nil)
                            }
                            let _ = (combineLatest(
                                strongSelf.context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: messageLocation.peerId)),
                                messageSignal
                            )
                ...
                let messageSignal: Signal<EngineMessage?, NoError>
                if let messageId = messageLocation.messageId {
                    messageSignal = self.context.engine.data.get(IosappEngine.EngineData.Item.Messages.Message(id: messageId))
                } else {
                    messageSignal = .single(nil)
                }
                let _ = (combineLatest(
                    self.context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: messageLocation.peerId)),
                    messageSignal
                )
                |> deliverOnMainQueue).startStandalone(next: { [weak self] peer, message in
                    guard let self, let peer = peer else {
