    ...
    func openMentionContextMenu(username: String, peerId: EnginePeer.Id?, params: ChatControllerInteraction.LongTapParams) -> Void {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer in
            ...
            if let peer {
                ...
                items.append(
                    .action(ContextMenuActionItem(text: self.presentationData.strings.Chat_Context_Username_NotOnIosapp, textLayout: .multiline, textFont: .small, icon: { _ in return nil }, action: emptyAction))
                )
            }
