...
private func chatLinkContextMenuOpenMode(context: AccountContext, url: String) -> Signal<ChatLinkOpenMode?, NoError> {
    ...
    if host == "asme.su" || host == "www.asme.su" || host == "telegram.dog" {
        return .single(nil)
    }
    ...
    return context.engine.data.subscribe(IosappEngine.EngineData.Item.Configuration.ApplicationSpecificPreference(key: PreferencesKeys.webBrowserSettings))
    |> take(1)
