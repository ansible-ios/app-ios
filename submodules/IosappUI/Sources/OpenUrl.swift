...
private func makeIosappUrl(_ path: String, queryItems: [URLQueryItem] = []) -> String {
    return appendQueryItems(to: "https://asme.su\(path)", items: queryItems)
}
...
func openExternalUrlImpl(context: AccountContext, urlContext: OpenURLContext, url: String, forceExternal: Bool, presentationData: PresentationData, navigationController: NavigationController?, dismissInput: @escaping () -> Void) {
    ...
    let continueHandling: () -> Void = {
        if let scheme = parsedUrl.scheme, (scheme == "as" || scheme == context.sharedContext.applicationBindings.appSpecificScheme) {
            ...
            if let query = parsedUrl.query, let params = QueryParameters(query) {
                ...
                case "user":
                    if let idValue = params["id"].flatMap(Int64.init), idValue > 0 {
                        let _ = (context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: EnginePeer.Id(namespace: Namespaces.Peer.CloudUser, id: EnginePeer.Id.Id._internalFromInt64Value(idValue))))
                ...
                case "textStyle":
                    if let slug = params["slug"] {
                        convertedUrl = makeIosappUrl("/addstyle/\(slug)")
                    }
                default:
                    break
                }
