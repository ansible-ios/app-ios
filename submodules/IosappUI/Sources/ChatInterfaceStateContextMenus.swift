...
func contextMenuForChatPresentationInterfaceState(chatPresentationInterfaceState: ChatPresentationInterfaceState, context: AccountContext, messages: [EngineRawMessage], controllerInteraction: ChatControllerInteraction?, selectAll: Bool, interfaceInteraction: ChatPanelInterfaceInteraction?, readStats: MessageReadStats? = nil, messageNode: ChatMessageItemView? = nil) -> Signal<ContextController.Items, NoError> {
    ...
    if messages.count == 1, let adAttribute = messages[0].adAttribute {
        ...
        if adAttribute.canReport {
            ...
            if let author = message.author, let addressName = author.addressName {
                let link = "https://asme.su/\(addressName)"
    ...
    var loadCopyMediaResource: IosappMediaResource?
    var isAction = false
    ...
    |> map { data, updatingMessageMedia, infoSummaryData, appConfig, isMessageRead, messageViewsPrivacyTips, availableReactions, translationSettings, loggingSettings, notificationSoundList, accountPeer -> ContextController.Items in
        ...
        if !messageText.isEmpty || richMessageMarkdown != nil || (resourceAvailable && isImage) || diceEmoji != nil {
            if !isExpired {
                ...
                } else if let message = messages.first, let peer = message.forwardInfo?.source as? IosappChannel, !(peer.addressName ?? "").isEmpty {
                    showTranslateIfTopical = true
                }
        ...
        if !hasViewStats, messages[0].forwardInfo == nil {
            for media in message.media {
                if let poll = media as? IosappMediaPoll, message.id.namespace == Namespaces.Message.Cloud, poll.pollId.namespace == Namespaces.Media.CloudPoll, poll.results.canViewStats {
            ...
            if hasReadReports || reactionCount != 0 {
                ...
                actions.insert(.custom(ChatReadReportContextItem(context: context, message: message, hasReadReports: hasReadReports, isEdit: false, stats: readStats, action: { c, f, stats, customReactionEmojiPacks, firstCustomEmojiReaction in
                    if message.id.peerId.namespace == Namespaces.Peer.CloudUser {
                        ...
                        if let channel = message.peers[message.id.peerId] as? IosappChannel, channel.hasPermission(.deleteAllMessages) {
        ...
        for media in message.media {
            if let poll = media as? IosappMediaPoll, message.id.namespace == Namespaces.Message.Cloud, poll.pollId.namespace == Namespaces.Media.CloudPoll {
...
func chatAvailableMessageActionsImpl(engine: IosappEngine, accountPeerId: EnginePeer.Id, messageIds: Set<EngineMessage.Id>, messages: [EngineMessage.Id: EngineRawMessage] = [:], peers: [EnginePeer.Id: EngineRawPeer] = [:], keepUpdated: Bool) -> Signal<ChatAvailableMessageActions, NoError> {
...
final class ChatReadReportContextItem: ContextMenuCustomItem {
    ...
    init(context: AccountContext, message: EngineRawMessage, hasReadReports: Bool, isEdit: Bool, stats: MessageReadStats?, action: ((ContextControllerProtocol, @escaping (ContextMenuActionResult) -> Void, MessageReadStats?, [StickerPackCollectionInfo], IosappMediaFile?) -> Void)?) {
        self.context = context
        self.message = message
