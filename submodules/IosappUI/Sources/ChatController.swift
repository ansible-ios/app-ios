    ...
    public init(
        ...
        clickThroughMessage: { [weak self] view, location in
            ...
            if peerId?.namespace != Namespaces.Peer.SecretChat, let interactiveEmojis = strongSelf.chatDisplayNode.interactiveEmojis, interactiveEmojis.emojis.contains(text) {
                strongSelf.sendMessages([.message(text: "", attributes: [], inlineStickers: [:], mediaReference: AnyMediaReference.standalone(media: IosappMediaDice(emoji: text)), threadId: strongSelf.chatLocation.threadId, replyToMessageId: replyMessageSubject, replyToStoryId: nil, localGroupingKey: nil, correlationId: nil, bubbleUpEmojiOrStickersets: [])])
            } else {
            ...
            strongSelf.presentPaidMessageAlertIfNeeded(completion: { [weak strongSelf] postpone in
                ...
                if silentPosting {
                    ...
                    if strongSelf.chatLocation.threadId == nil, let user = strongSelf.presentationInterfaceState.renderedPeer?.peer as? IosappUser, let botInfo = user.botInfo, botInfo.flags.contains(.hasForum), botInfo.flags.contains(.forumManagedByUser) {
            ...
            if let strongSelf = self {
                ...
                let urlAuthAccountPeer = strongSelf.context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: strongSelf.context.account.peerId))
            ...
            if id.namespace == Namespaces.Message.EphemeralLocal {
                let _ = (strongSelf.context.engine.data.get(IosappEngine.EngineData.Item.Messages.Message(id: id))
                |> deliverOnMainQueue).startStandalone(next: { [weak self] message in
                    guard let strongSelf = self, let message else {
                        return
                    }
    ...
    func openHashtag(_ hashtag: String, peerName: String?) {
        let _ = self.presentVoiceMessageDiscardAlert(action: {
            ...
            if let peerName = peerName {
                ...
                resolveSignal = self.context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: peerId))
                |> mapToSignal { peer -> Signal<EnginePeer, NoError> in
                    if let peer {
                        return .single(peer)
                    } else {
    ...
    func openResolved(result: ResolvedUrl, sourceMessageId: MessageId?, progress: Promise<Bool>? = nil, forceExternal: Bool = false, concealed: Bool = false, commit: @escaping () -> Void = {}) {
        ...
        self.context.sharedContext.openResolvedUrl(result, context: self.context, urlContext: urlContext, navigationController: self.effectiveNavigationController, forceExternal: forceExternal, forceUpdate: false, openPeer: { [weak self] peerId, navigation in
            ...
            switch navigation {
                ...
                case .info:
                    dismissWebAppControllers()
                    strongSelf.navigationActionDisposable.set((strongSelf.context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: peerId.id))
                    |> mapToSignal { peer -> Signal<EnginePeer, NoError> in
                        if let peer {
                            return .single(peer)
                        } else {
