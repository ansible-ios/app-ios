        ...
        if peerId.namespace == Namespaces.Peer.CloudUser || peerId.namespace == Namespaces.Peer.SecretChat {
            ...
            for entry in view.additionalData {
                if case let .peer(_, value) = entry {
                    if let channel = value as? IosappChannel {
...
public final class ChatHistoryListNodeImpl: ASDisplayNode, ChatHistoryNode, ChatHistoryListNode {
                ...
                if context.sharedContext.immediateExperimentalUISettings.fakeAds {
                    ...
                    |> map { peer -> (interPostInterval: Int32?, messages: [Message], startDelay: Int32?, betweenDelay: Int32?) in
                        let fakeAdMessages: [Message] = (0 ..< 10).map { i -> Message in
                            ...
                            attributes.append(AdMessageAttribute(opaqueId: "fake_ad_\(i)".data(using: .utf8)!, messageType: mappedMessageType, url: "asme.su/telegram", buttonText: "VIEW", sponsorInfo: nil, additionalInfo: nil, canReport: false, hasContentMedia: false, minDisplayDuration: nil, maxDisplayDuration: nil))
    ...
    private func beginPresentationDataManagement(updated: Signal<PresentationData, NoError>) {
        let appConfiguration = self.context.engine.data.subscribe(IosappEngine.EngineData.Item.Configuration.ApplicationSpecificPreference(key: PreferencesKeys.appConfiguration))
        |> take(1)
        |> map { view in
