...
public final class AccountContextImpl: AccountContext {
    ...
    public init(sharedContext: SharedAccountContextImpl, account: Account, limitsConfiguration: LimitsConfiguration, contentSettings: ContentSettings, appConfiguration: AppConfiguration, availableReplyColors: EngineAvailableColorOptions, availableProfileColors: EngineAvailableColorOptions, temp: Bool = false)
    {
        ...
        let updatedLimitsConfiguration = self.engine.data.subscribe(IosappEngine.EngineData.Item.Configuration.ApplicationSpecificPreference(key: PreferencesKeys.limitsConfiguration))
    ...
    public func chatLocationOutgoingReadState(for location: ChatLocation, contextHolder: Atomic<ChatLocationContextHolder?>) -> Signal<MessageId?, NoError> {
        ...
        case let .replyThread(data):
            if data.isForumPost, let peerId = location.peerId {
                return self.engine.data.subscribe(IosappEngine.EngineData.Item.Messages.ThreadInfo(peerId: data.peerId, threadId: data.threadId))
    ...
    public func chatLocationUnreadCount(for location: ChatLocation, contextHolder: Atomic<ChatLocationContextHolder?>) -> Signal<Int, NoError> {
        switch location {
        case let .peer(peerId):
            return self.engine.data.subscribe(IosappEngine.EngineData.Item.Messages.PeerUnreadCount(id: peerId, handleThreads: false))
        case let .replyThread(data):
            if data.isForumPost {
                return self.engine.data.subscribe(IosappEngine.EngineData.Item.Messages.ThreadInfo(peerId: data.peerId, threadId: data.threadId))
...
func getAppConfiguration(engine: IosappEngine) -> Signal<AppConfiguration, NoError> {
    return engine.data.subscribe(IosappEngine.EngineData.Item.Configuration.ApplicationSpecificPreference(key: PreferencesKeys.appConfiguration))
