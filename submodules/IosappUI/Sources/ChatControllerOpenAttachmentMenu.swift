...
extension ChatControllerImpl {
    ...
    func presentAttachmentMenu(subject: AttachMenuSubject) {
        Task { @MainActor [weak self] in
            ...
            struct RichTextDraft: Codable {
                ...
                let emojiFiles: [Int64: IosappMediaFile]

                init(document: RichTextAttachmentScreen.Document, media: [String: Media], emojiFiles: [Int64: IosappMediaFile]) {
                ...
                init(from decoder: Decoder) throws {
                    ...
                    // `Media`/`IosappMediaFile` are Postbox-coded (not `Codable`), so each value is stored as a
                    ...
                    for i in 0 ..< emojiFilesKeys.count {
                        guard let emojiFileValue = PostboxDecoder(buffer: MemoryBuffer(data: emojiFilesValues[i])).decodeRootObject() as? IosappMediaFile else {
                            throw LoadError.generic
                        }
            ...
            if let peerId = self.chatLocation.peerId {
                ...
                if let entry = await self.context.engine.data.get(
                    IosappEngine.EngineData.Item.ItemCache.Item(collectionId: Namespaces.CachedItemCollection.richTextComposerDrafts, id: key)
                ).get() {
    ...
    func presentRichTextAttachmentMenu(photoVideoOnly: Bool, completion: @escaping (RichTextAttachmentScreen.RichTextAttachment) -> Void) {
        ...
        presentPollAttachmentScreen(context: self.context, updatedPresentationData: self.updatedPresentationData, subject: .richText, availableButtons: availableButtons, inputMediaNodeData: nil, present: { [weak self] c, push in
            ...
            if let image = result.concrete(IosappMediaImage.self) {
                completion(.image(image))
            } else if let file = result.concrete(IosappMediaFile.self) {
                completion(.file(file))
            } else if let mapReference = result.concrete(IosappMediaMap.self) {
