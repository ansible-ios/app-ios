...
private enum CommunityAddChatsMode: Equatable {
    ...
    init(defaultBannedRights: IosappChatBannedRights?) {
...
private struct CommunityPendingPeerAvatar {
    let previewRepresentation: IosappMediaImageRepresentation
...
private enum CommunityCreateAvatarSetup {
    private static func makePhotoRepresentation(context: AccountContext, image: UIImage) -> (LocalFileMediaResource, IosappMediaImageRepresentation)? {
        ...
        let representation = IosappMediaImageRepresentation(
    ...
    static func video(
        ...
        |> mapToSignal { exportSubject, duration -> Signal<IosappMediaResource?, UploadPeerPhotoError> in
...
private final class CommunityEditScreenComponent: Component {
    ...
    final class View: UIView, UIScrollViewDelegate {
        ...
        private var community: IosappCommunity?
        ...
        private func ensureDataSignal(component: CommunityEditScreenComponent) {
            ...
            self.dataDisposable = (combineLatest(
                component.context.engine.data.subscribe(IosappEngine.EngineData.Item.Peer.Peer(id: communityId)),
                component.context.engine.data.subscribe(IosappEngine.EngineData.Item.Peer.CachedData(id: communityId))
            )
        ...
        private func updateLinkedPeerSignals(component: CommunityEditScreenComponent, ids: [EnginePeer.Id]) {
            ...
            self.linkedPeersDisposable.set((component.context.engine.data.subscribe(
                EngineDataMap(ids.map(IosappEngine.EngineData.Item.Peer.Peer.init(id:)))
            )
            ...
            self.linkedPeerDataDisposable.set((component.context.engine.data.subscribe(
                EngineDataMap(ids.map(IosappEngine.EngineData.Item.Peer.CachedData.init(id:)))
            )
        ...
        private func rightsWithUpdatedMode(_ mode: CommunityAddChatsMode, current: IosappChatBannedRights?) -> IosappChatBannedRights {
            var flags = current?.flags ?? IosappChatBannedRightsFlags()
            switch mode {
            case .allMembers:
            ...
            return IosappChatBannedRights(flags: flags, untilDate: current?.untilDate ?? Int32.max)
        }
        ...
        func update(component: CommunityEditScreenComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<EnvironmentType>, transition: ComponentTransition) -> CGSize {
            ...
            if let community = self.community {
                ...
                avatarPeer = EnginePeer(IosappCommunity(
