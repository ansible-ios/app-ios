    ...
    final class View: UIView, UIScrollViewDelegate {
        ...
        private func ensureSubjectPeerSignal(component: CommunitiesScreenComponent) {
            ...
            if self.subjectPeerDisposable == nil {
                self.subjectPeerDisposable = (component.context.engine.data.subscribe(IosappEngine.EngineData.Item.Peer.Peer(id: peerId))
                ...
                |> mapToSignal { state -> Signal<[CommunityListEntry], NoError> in
                    ...
                    return combineLatest(
                        component.context.engine.data.subscribe(
                            EngineDataMap(communityIds.map(IosappEngine.EngineData.Item.Peer.Peer.init(id:)))
                        ),
                        component.context.engine.data.subscribe(
                            EngineDataMap(communityIds.map(IosappEngine.EngineData.Item.Peer.CachedData.init(id:)))
                        )
                    )
