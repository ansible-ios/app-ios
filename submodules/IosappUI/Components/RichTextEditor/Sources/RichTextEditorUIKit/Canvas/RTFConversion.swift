...
enum RTFConversion {
    ...
    // submodules/TextFormat/Sources/CustomEmojiMarkdownMarker.swift — `as://emoji?id=<fileId>`.
    private static let emojiMarkerPrefix = "as://emoji?id="
    /// `as://emoji?id=<id>&n=<seq>`. The `&n=` suffix is a per-export sequence number that makes EVERY
    ...
    private static func runs(in attr: NSAttributedString, range: NSRange) -> [TextRun] {
        ...
        attr.enumerateAttributes(in: range, options: []) { dict, sub, _ in
            ...
            if let linkString, let emojiID = emojiID(fromMarkerURL: linkString) {
                // A as://emoji?id= marker → reconstruct a single-U+FFFC custom emoji run (the model
