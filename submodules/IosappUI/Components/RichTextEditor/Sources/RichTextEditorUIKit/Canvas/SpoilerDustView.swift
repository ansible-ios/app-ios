...
/// The animated "dust" cloud drawn over one hidden spoiler run (the Iosapp invisible-ink effect). Ported
/// from Iosapp's `InvisibleInkDustNode`/`InvisibleInkDustView`
...
/// Iosapp helpers → UIKit equivalents. The editor restores the hidden text via a rendering attribute (no
/// `textNode`), so the text-mask half of Iosapp's reveal is omitted; the emitter + its mask are kept
...
final class SpoilerDustView: UIView {
    /// Hosts the emitter layer; gets the radial reveal mask (mirrors Iosapp's `emitterNode`).
    ...
    /// to where ink is (Iosapp's `spoilerWords`), `lineRects` are the hit-test area. All in the view's own
    /// coordinates.
    ...
    /// dissolves the dust outward from that point (Iosapp's `revealAtLocation`). With `nil` (a caret-driven
    ...
    func dissolve(explodingAt point: CGPoint?, completion: @escaping () -> Void) {
        ...
        // Build the inverse radial mask off the main thread (matches Iosapp), then install + animate it.
        ...
        // mask dissolve, drop the attractor + the view. Mirrors Iosapp's 0.8s post-reveal cleanup.
    ...
    /// The inverse radial-gradient mask spot (transparent at `position`, opaque outward) — Iosapp's
    ...
    /// Iosapp's actual particle texture (`textSpeckle_Normal.png`, a 4×4 soft gray+alpha speckle), bundled
    ...
    /// The bundled Iosapp particle texture, resolved from THIS module's resource bundle (`.module` here
