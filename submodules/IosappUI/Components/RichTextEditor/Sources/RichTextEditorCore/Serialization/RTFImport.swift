...
final class RTFDocumentParser {
    ...
    // Custom emoji as:// URL prefix (keep in sync with TextFormat/CustomEmojiMarkdownMarker.swift).
    private static let emojiPrefix = "as://emoji?id="
    ...
    private func handle(_ tok: RTFToken) {
        ...
        case .groupEnd:
            ...
            if state.inFieldResult && stack.count == fldRsltDepth {
                // If the resultURL is a as://emoji marker, convert the accumulated
    ...
    /// If `url` is a `as://emoji?id=<id>` marker, returns the id (up to first `&`).
