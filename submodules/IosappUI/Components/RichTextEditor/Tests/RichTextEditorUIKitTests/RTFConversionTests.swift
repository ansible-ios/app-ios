...
final class RTFConversionTests: XCTestCase {
    ...
    // MARK: Custom emoji cross-app via as://emoji?id= (spec addendum 2026-06-24)
    ...
    func test_export_emoji_emitsAltTextHyperlinkedToMarker() throws {
        ...
        XCTAssertEqual(urlString?.hasPrefix("as://emoji?id=12345"), true)
    }

    func test_import_emojiMarkerLink_reconstructsSingleObjectReplacementRun() throws {
        // RTF carrying a as://emoji?id= hyperlink on ":star:" text (what another app would round-trip).
        let attr = NSAttributedString(string: ":star:", attributes: [
            .font: UIFont.systemFont(ofSize: 17),
            .link: URL(string: "as://emoji?id=99")!,
        ])
    ...
    func test_inlineRTF_emitsToggles_link_andEmojiMarker() {
        ...
        XCTAssertTrue(rtf.contains("HYPERLINK \"as://emoji?id=7&n=0\""))       // emoji marker (first dedup seq)
