...
final class ComposePollScreenComponent: Component {
    ...
    final class AttachedMedia {
        ...
        var requiresUpload: Bool {
            ...
            if let webpage = self.media.media as? IosappMediaWebpage, webpage.id?.namespace == Namespaces.Media.LocalFile {
                return true
            }
            return false
        }
    ...
    final class View: UIView, UIScrollViewDelegate {
        ...
        private func openAttachedMedia(subject: MediaAttachSubject, replace: Bool = false) {
            ...
            if !replace, case .pollOption = subject, let webpage = self.attachedMedia(for: subject)?.media.media as? IosappMediaWebpage, let link = webpage.content.url {
        ...
        private func uploadAttachedMediaIfNeeded(_ media: AttachedMedia) {
            ...
            if media.media.media is IosappMediaWebpage {
            } else {
                media.progress = 0.0
            }
            ...
            if let webpage = media.media.media as? IosappMediaWebpage, case let .Loaded(content) = webpage.content {
