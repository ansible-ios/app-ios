        ...
        func updateContent(component: StickerAttachmentScreenComponent) {
            self.emojiContent?.inputInteractionHolder.inputInteraction = EmojiPagerContentComponent.InputInteraction(
                performItemAction: { [weak self] groupId, item, _, _, _, _ in
                    ...
                    if groupId == AnyHashable("featuredTop"), let file = item.itemFile {
                        ...
                        |> deliverOnMainQueue).start(next: { [weak self] hasPremium, hasGlobalPremium in
                            ...
                            let _ = (combineLatest(
                                context.engine.data.subscribe(IosappEngine.EngineData.Item.ItemCollections.InstalledPackIds(namespace: Namespaces.ItemCollection.CloudEmojiPacks)),
                                context.engine.data.subscribe(IosappEngine.EngineData.Item.OrderedLists.ListItems(collectionId: Namespaces.OrderedItemList.CloudFeaturedEmojiPacks))
                            )
                            |> take(1)
                ...
                addGroupAction: { [weak self] groupId, isPremiumLocked, _ in
                    ...
                    let context = component.context
                    let _ = (context.engine.data.subscribe(IosappEngine.EngineData.Item.OrderedLists.ListItems(collectionId: Namespaces.OrderedItemList.CloudFeaturedEmojiPacks))
                    |> take(1)
            ...
            self.stickerContent?.inputInteractionHolder.inputInteraction = EmojiPagerContentComponent.InputInteraction(
                performItemAction: { [weak self] groupId, item, _, _, _, _ in
                    ...
                    if groupId == AnyHashable("featuredTop") {
                        let _ = (context.engine.data.subscribe(IosappEngine.EngineData.Item.OrderedLists.ListItems(collectionId: Namespaces.OrderedItemList.CloudFeaturedStickerPacks))
                        |> take(1)
