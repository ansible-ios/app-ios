        ...
        func setup(
            ...
            case .message, .gift:
                ...
                |> deliverOnMainQueue).start(next: { [weak self] messages in
                    ...
                    if let maybeFile = messages.first?.media.first(where: { $0 is IosappMediaFile }) as? IosappMediaFile, maybeFile.isVideo, let _ = self.context.engine.resources.completedResourcePath(id: EngineMediaResource.Id(maybeFile.resource.id), pathExtension: nil) {
                    ...
                    if let resource = resource as? CloudDocumentMediaResource {
                        ...
                        |> mapToSignal { status -> Signal<(UploadStickerStatus, (StickerPackReference, String)?), UploadStickerError> in
                            ...
                            case let .complete(resource, _):
                                let rawResource = resource._asResource() as! IosappMediaResource
...
private func allowedStoryReactions(context: AccountContext) -> Signal<[ReactionItem], NoError> {
    let topReactions = context.engine.data.subscribe(IosappEngine.EngineData.Item.OrderedLists.ListItems(collectionId: Namespaces.OrderedItemList.CloudTopReactions))
