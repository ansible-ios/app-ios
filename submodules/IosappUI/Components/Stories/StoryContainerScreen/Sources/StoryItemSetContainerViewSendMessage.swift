    ...
    func performShareAction(view: StoryItemSetContainerComponent.View) {
        ...
        if focusedItem.storyItem.isForwardingDisabled {
            ...
            let shareController = component.context.sharedContext.makeShareController(context: component.context, params: ShareControllerParams(
                subject: .media(AnyMediaReference.standalone(media: IosappMediaStory(storyId: EngineStoryId(peerId: peerId, id: focusedItem.storyItem.id), isMention: false)), nil),
...
//        component.controller()?.push(controller)
    ...
    func openResolved(view: StoryItemSetContainerComponent.View, result: ResolvedUrl, forceExternal: Bool = false, concealed: Bool = false, completion: (() -> Void)? = nil) {
        ...
        component.context.sharedContext.openResolvedUrl(
            ...
            openPeer: { [weak view] peerId, navigation in
                ...
                case .info:
                    let _ = (context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: peerId.id))
    ...
    func openHashtag(view: StoryItemSetContainerComponent.View, hashtag: String, peerName: String?) {
        ...
        if let peerName = peerName {
            ...
            resolveSignal = component.context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: peerId))
            |> mapToSignal { peer -> Signal<EnginePeer?, NoError> in
                if let peer {
                    return .single(peer)
                } else {
    ...
    func openPeer(view: StoryItemSetContainerComponent.View, peer: EnginePeer, expandAvatar: Bool = false, peerTypes: ReplyMarkupButtonAction.PeerTypes? = nil) {
        ...
        let peerSignal: Signal<EnginePeer?, NoError> = component.context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: peer.id))
        |> mapToSignal { peer -> Signal<EnginePeer, NoError> in
            if let peer {
                return .single(peer)
            } else {
    ...
    func activateMediaArea(view: StoryItemSetContainerComponent.View, mediaArea: MediaArea, position: CGPoint? = nil, immediate: Bool = false) {
        ...
        case let .starGift(_, slug):
            useGesturePosition = true
            let action = {
                let _ = component.context.sharedContext.openUserGeneratedUrl(context: component.context, peerId: nil, url: "https://asme.su/nft/\(slug)", webpage: nil, concealed: false, forceConcealed: false, skipUrlAuth: false, skipConcealedAlert: false, forceDark: true, present: { [weak controller] c in
                    controller?.present(c, in: .window(.root))
                }, openResolved: { [weak self, weak view] resolved in
                    guard let self, let view else {
                        return
