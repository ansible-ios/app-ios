...
final class TextProcessingContentComponent: Component {
    ...
    let displayLanguageSelectionMenu: (UIView, String, IosappComposeAIMessageMode.StyleId, Bool,  @escaping (String, IosappComposeAIMessageMode.StyleReference) -> Void) -> Void
    let newStyleAdded: (IosappComposeAIMessageMode.CloudStyle) -> Void
    let styleUpdated: (IosappComposeAIMessageMode.CloudStyle) -> Void
    let styleDeleted: (IosappComposeAIMessageMode.StyleId) -> Void
    ...
    init(
        ...
        displayLanguageSelectionMenu: @escaping (UIView, String, IosappComposeAIMessageMode.StyleId, Bool, @escaping (String, IosappComposeAIMessageMode.StyleReference) -> Void) -> Void,
        newStyleAdded: @escaping (IosappComposeAIMessageMode.CloudStyle) -> Void,
        styleUpdated: @escaping (IosappComposeAIMessageMode.CloudStyle) -> Void,
        styleDeleted: @escaping (IosappComposeAIMessageMode.StyleId) -> Void,
    ...
    final class View: UIView {
        ...
        private func requestShareStyle(id: IosappComposeAIMessageMode.StyleReference) {
            guard let component = self.component else {
                return
            }
            ...
            let shareController = component.context.sharedContext.makeShareController(context: component.context, params: ShareControllerParams(
                subject: .url("https://asme.su/addstyle/\(slug)"),
                completed: { [weak self] peerIds in
                    Task { @MainActor in
                        ...
                        if peerId == component.context.account.peerId {
                            ...
                            guard let peer = await component.context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: peerId)).get() else {
                                return
                            }
        ...
        private func requestEditStyle(id: IosappComposeAIMessageMode.StyleReference) {
        ...
        private func requestDeleteStyle(id: IosappComposeAIMessageMode.StyleReference) {
            guard let component = self.component, let environment = self.environment else {
                return
            }
            guard let style = component.styles.first(where: { .style($0.reference.id) == id.id }) else {
        ...
        private func openStyleContextMenu(id: IosappComposeAIMessageMode.StyleReference, gesture:  ContextGesture, sourceView: ContextExtractedContentContainingView) {
            guard let component = self.component, let environment = self.environment else {
                return
            }
        ...
        private func requestStylePreview() {
            ...
            self.currentStylePreviewDisposable?.dispose()
            self.currentStylePreviewDisposable = (component.context.engine.messages.requestAIMessageStylePreview(reference: IosappComposeAIMessageMode.CloudStyle(content: .custom(style)).reference, index: index) |> deliverOnMainQueue).startStrict(next: { [weak self] result in
                    ...
                    createStyle: { [weak self] in
                        Task { @MainActor in
                            ...
                            let userLimits = await component.context.engine.data.get(
                                IosappEngine.EngineData.Item.Configuration.UserLimits(isPremium: hasPremium)
                            ).get()
...
private final class TextProcessingSheetComponent: Component {
    ...
    final class View: UIView {
        ...
        private final class LanguageSelectionMenuData {
            ...
            let completion: (String, IosappComposeAIMessageMode.StyleReference) -> Void

            init(sourceView: UIView, currentLanguage: String, currentStyle: IosappComposeAIMessageMode.StyleId, displayStyle: Bool, completion: @escaping (String, IosappComposeAIMessageMode.StyleReference) -> Void) {
        ...
        private var styleCreatedToastData: (timer: Foundation.Timer, emojiFile: IosappMediaFile, style: IosappComposeAIMessageMode.CloudStyle.Custom)?
                            ...
                            Task { @MainActor in
                                ...
                                if case let .custom(style) = style.content, let authorId = style.authorId {
                                    authorPeer = await component.context.engine.data.get(
                                        IosappEngine.EngineData.Item.Peer.Peer(id: authorId)
                                    ).get()
                                }
...
public class TextProcessingScreen: ViewControllerComponentContainer {
    ...
    public final class Style: Equatable {
        public let reference: IosappComposeAIMessageMode.CloudStyle.Reference
        public let title: String
        public let emojiFileId: Int64?
        public let emojiFile: IosappMediaFile?
        public let isAuthor: Bool
        public let slug: String?
        public let cloudStyle: IosappComposeAIMessageMode.CloudStyle
        public let authorPeer: EnginePeer?
        
        public var id: IosappComposeAIMessageMode.StyleReference {
            return .style(self.reference)
        }
        
        public init(reference: IosappComposeAIMessageMode.CloudStyle.Reference, title: String, emojiFileId: Int64?, emojiFile: IosappMediaFile?, isAuthor: Bool, slug: String?, cloudStyle: IosappComposeAIMessageMode.CloudStyle, authorPeer: EnginePeer?) {
        ...
        convenience init(cloudStyle: IosappComposeAIMessageMode.CloudStyle, authorPeer: EnginePeer?) {
            let title: String
    ...
    public init(
        ...
        var styles: [Style] = []
        let resolvedEmojiFiles: [Int64: IosappMediaFile] = await context.engine.stickers.resolveInlineStickersLocal(fileIds: Array(Set(rawStyles.compactMap({ style in
        ...
        var previewIconFile: IosappMediaFile?
