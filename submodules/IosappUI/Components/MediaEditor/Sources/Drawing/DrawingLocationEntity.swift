...
public final class DrawingLocationEntity: DrawingEntity, Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let locationData = try container.decodeIfPresent(Data.self, forKey: .location) {
            self.location = EnginePostboxDecoder(buffer: EngineMemoryBuffer(data: locationData)).decodeRootObject() as! IosappMediaMap
        } else {
            fatalError()
        }
        
        if let iconData = try container.decodeIfPresent(Data.self, forKey: .icon) {
            self.icon = EnginePostboxDecoder(buffer: EngineMemoryBuffer(data: iconData)).decodeRootObject() as? IosappMediaFile
        }
