        ...
        case let .message(messageId):
            ...
            |> mapToSignal { message in
                ...
                if let message, !"".isEmpty {
                    if let maybeFile = message.media.first(where: { $0 is IosappMediaFile }) as? IosappMediaFile, maybeFile.isVideo, let path = self.context.engine.resources.completedResourcePath(id: EngineMediaResource.Id(maybeFile.resource.id), pathExtension: "mp4") {
                        let asset = AVURLAsset(url: URL(fileURLWithPath: path))
