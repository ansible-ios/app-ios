        ...
        case let .media(mediaBlock):
            ...
            if mediaBlock.items.count >= 2 {
                ...
                case .image, .video, .audio:
                    // Media.id is optional on the protocol; real IosappMedia* image/video/audio always return non-nil.
                ...
                case .location:
                    // A location resolves to a `IosappMediaMap` (id-less); emit a `.map` block with the coordinates
                    // inline. No `pageMedia` entry (the block carries no MediaId). See ChatInputContentInstantPage.
                    if let map = resolved as? IosappMediaMap {
...
/// `IosappCore/Sources/ChatInputContent/ChatInputContentInstantPage.swift` (the composer's already-fixed
