...
final class PeerInfoLinkedCommunityData {
    let peer: IosappCommunity
    let cachedData: CachedCommunityData?

    init(peer: IosappCommunity, cachedData: CachedCommunityData?) {
        self.peer = peer
...
func peerInfoScreenSettingsData(context: AccountContext, peerId: EnginePeer.Id, accountsAndPeers: Signal<[(AccountContext, EnginePeer, Int32)], NoError>, activeSessionsContextAndCount: Signal<(ActiveSessionsContext, Int, WebSessionsContext)?, NoError>, notificationExceptions: Signal<NotificationExceptionsList?, NoError>, privacySettings: Signal<AccountPrivacySettings?, NoError>, archivedStickerPacks: Signal<[ArchivedStickerPackItem]?, NoError>, hasPassport: Signal<Bool, NoError>, starsContext: StarsContext?, tonContext: StarsContext?) -> Signal<PeerInfoScreenData, NoError> {
    ...
    |> mapToSignal { botPeerId -> Signal<EnginePeer?, NoError> in
        ...
        return context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: botPeerId))
    }
    ...
    return combineLatest(
        ...
        hasPassport,
        context.engine.data.subscribe(IosappEngine.EngineData.Item.Configuration.ApplicationSpecificPreference(key: PreferencesKeys.appConfiguration)),
    ...
    |> mapToSignal { inputData -> Signal<PeerInfoScreenData, NoError> in
        ...
        case let .user(userPeerId, secretChatId, kind):
            ...
            if case .bot = kind {
                ...
                webAppPermissions = context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: peerId))
            ...
            |> mapToSignal { peerView, availablePanes, globalNotificationSettings, encryptionKeyFingerprint, status, hasStories, hasStoryArchive, recommendedBots, accountIsPremium, savedMessagesPeer, hasSavedMessagesChats, hasSavedMessages, hasSavedMessageTags, hasBotPreviewItems, personalChannel, privacySettings, starsRevenueContextAndState, revenueContextAndState, premiumGiftOptions, webAppPermissions, savedMusicState, businessConnectedBot -> Signal<PeerInfoScreenData, NoError> in
                ...
                if let user = peerViewMainPeer(peerView) as? IosappUser, let linkedCommunityId = user.linkedCommunityId {
                    ...
                    linkedCommunityData = combineLatest(
                        context.engine.data.subscribe(IosappEngine.EngineData.Item.Peer.Peer(id: linkedCommunityId)),
                        context.engine.data.subscribe(IosappEngine.EngineData.Item.Peer.CachedData(id: linkedCommunityId))
                    )
        ...
        case .channel:
            ...
            |> mapToSignal { peerView, availablePanes, globalNotificationSettings, status, currentInvitationsContext, invitations, currentRequestsContext, requests, hasStories, accountIsPremium, recommendedChannels, hasSavedMessages, hasSavedMessagesChats, hasSavedMessageTags, isPremiumRequiredForStoryPosting, starsRevenueContextAndState, revenueContextAndState, profileGiftsState, personalChannel -> Signal<PeerInfoScreenData, NoError> in
                ...
                let linkedCommunityData: Signal<PeerInfoLinkedCommunityData?, NoError>
                if let channel = peerViewMainPeer(peerView) as? IosappChannel, let linkedCommunityId = channel.linkedCommunityId {
                    let previousLinkedCommunityId = forcedLinkedCommunityId.swap(linkedCommunityId)
                    if previousLinkedCommunityId != linkedCommunityId {
                        context.account.viewTracker.forceUpdateCachedPeerData(peerId: linkedCommunityId)
                    }
        ...
        case let .group(groupId):
            ...
            if case let .replyThread(message) = chatLocation {
                ...
                threadData = context.engine.data.subscribe(IosappEngine.EngineData.Item.Messages.ThreadInfo(peerId: peerId, threadId: threadId))
            } else {
...
func peerInfoIsChatMuted(peer: EnginePeer?, peerNotificationSettings: IosappPeerNotificationSettings?, threadNotificationSettings: IosappPeerNotificationSettings?, globalNotificationSettings: EngineGlobalNotificationSettings?) -> Bool {
    func isPeerMuted(peer: EnginePeer?, peerNotificationSettings: IosappPeerNotificationSettings?, globalNotificationSettings: EngineGlobalNotificationSettings?) -> Bool {
