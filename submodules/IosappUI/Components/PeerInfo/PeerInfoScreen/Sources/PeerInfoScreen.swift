...
final class PeerInfoScreenNode: ViewControllerTracingNode, PeerInfoScreenNodeProtocol, ASScrollViewDelegate {
    ...
    init(
        ...
        if let reactionSourceMessageId = self.reactionSourceMessageId {
            reactionSourceMessage = context.engine.data.subscribe(IosappEngine.EngineData.Item.Messages.Message(id: reactionSourceMessageId))
        } else {
        ...
        self.dataDisposable = combineLatest(
            ...
            var canDeleteReaction = false
            if let sourceMessage = reactionSourceMessage?._asMessage(), let reactionPeerId = data.peer?.id, let channel = sourceMessage.peers[sourceMessage.id.peerId] as? IosappChannel, channel.hasPermission(.deleteAllMessages) {
                ...
                let _ = self.context.account.postbox.transaction { transaction -> Void in
                    if case let .channel(channel) = peer {
                        updatePeersCustom(transaction: transaction, peers: [channel.withUpdatedLinkedCommunityId(nil)], update: { _, updated in
                            guard let channel = updated as? IosappChannel else {
                                return updated
                            }
