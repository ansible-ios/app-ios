...
public func isPollEffectivelyClosed(message: EngineMessage, poll: IosappMediaPoll) -> Bool {
    if poll.isClosed {
        return true
    } else {
        return false
...
public func messageIsEligibleForLargeCustomEmoji(_ message: EngineMessage) -> Bool {
    ...
    for entity in entities {
        if case let .CustomEmoji(_, fileId) = entity.type {
            if let _ = message.associatedMedia[EngineMedia.Id(namespace: Namespaces.Media.CloudFile, id: fileId)] as? IosappMediaFile {
