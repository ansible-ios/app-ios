...
public final class RichTextEditorChatInputNode: ASDisplayNode, ChatRichTextInputNode {
    ...
    /// (`document(from:)` / `document(fromChatInputContent:)` drop the `IosappMediaFile`), but the host
    ...
    public var attributedText: NSAttributedString? {
        ...
        set {
            ...
            // `IosappMediaFile` the renderer needs). Done BEFORE the equality guard so the cache stays
    ...
    /// caches and return the editor-side ref/key: `registerEmoji` records the `IosappMediaFile` so the editor's
    ...
    /// cache (the editor drops the `IosappMediaFile`), or nil if this emoji wasn't seen via a file-bearing push.
    ...
    private func resolveEmojiRef(_ emojiRef: EmojiRef) -> (fileId: Int64, file: IosappMediaFile?)? {
    ...
    /// Direct-bridge `registerEmoji`: cache the `IosappMediaFile` (mirroring the `attributedText`-setter
    ...
    private func registerEmojiRef(fileId: Int64, file: IosappMediaFile?) -> EmojiRef {
        if let file {
