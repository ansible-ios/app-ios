...
/// only (it has no `IosappMediaFile` slot), so the files ride alongside in `emojiFiles`; the article editor
...
public func documentMediaAndEmoji(fromChatInputContent content: ChatInputContent) -> (document: Document, media: [String: Media], emojiFiles: [Int64: IosappMediaFile]) {
    var media: [String: Media] = [:]
    var emojiFiles: [Int64: IosappMediaFile] = [:]
    let doc = document(
...
public func documentMediaAndEmojiAsync(engine: IosappEngine, fromChatInputContent content: ChatInputContent) async -> (document: Document, media: [String: Media], emojiFiles: [Int64: IosappMediaFile]) {
    var media: [String: Media] = [:]
    var emojiFiles: [Int64: IosappMediaFile] = [:]
...
/// unresolved key drops the block); a custom emoji's `IosappMediaFile` is resolved from `emojiFiles` (the
...
public func chatInputContent(fromDocument document: Document, media: [String: Media], emojiFiles: [Int64: IosappMediaFile]) -> ChatInputContent {
