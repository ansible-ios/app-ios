        ...
        case .tap:            
            ...
            if let item = self.item, let emojiString = self.emojiString, emojiString.emojis.count > 1 {
                ...
                let emojiTapAction: (Bool) -> InternalBubbleTapAction? = { shouldPlay in
                    ...
                    let appConfiguration = item.context.engine.data.subscribe(IosappEngine.EngineData.Item.Configuration.ApplicationSpecificPreference(key: PreferencesKeys.appConfiguration))
                    |> take(1)
                    |> map { view in
