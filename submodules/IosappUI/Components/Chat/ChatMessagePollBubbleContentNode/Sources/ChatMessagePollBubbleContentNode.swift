                ...
                return (CGSize(width: width, height: contentHeight), { animated, inProgress, attemptSynchronous in
                    ...
                    if let media, let mediaFrame, !isSticker {
                        ...
                        if let image = media as? IosappMediaImage, let largest = largestImageRepresentation(image.representations) {
                            ...
                            if mediaUpdated, let photoReference = mediaReference.concrete(IosappMediaImage.self) {
                            ...
                            if let fileReference = mediaReference.concrete(IosappMediaFile.self), mediaUpdated {
    ...
    private func updateAttachmentLayout(size: CGSize, tintColor: UIColor) {
        ...
        if let attachment = self.currentAttachment, let media = attachment.media, !isSticker, let context = self.currentContext, let theme = self.currentTheme {
            ...
            if let image = media.media as? IosappMediaImage, let largest = largestImageRepresentation(image.representations), let photoReference = media.concrete(IosappMediaImage.self) {
                ...
                self.appliedWebpagePlaceholderColor = nil
            } else if let webpage = media.media as? IosappMediaWebpage {
                let webpageReference = WebpageReference(webpage)
                if case let .Loaded(content) = webpage.content, let image = content.image {
                    if let largest = largestImageRepresentation(image.representations) {
                        imageSize = largest.dimensions.cgSize.aspectFilled(imageNodeSize)
                ...
                if optionNode.frame.contains(point) {
                    ...
                    if let mediaFrame = optionNode.mediaFrame {
                        ...
                        if absoluteMediaFrame.contains(point), let option = optionNode.option, let media = option.media {
                            if case .longTap = gesture, let webpage = media as? IosappMediaWebpage, let url = webpage.content.url {
