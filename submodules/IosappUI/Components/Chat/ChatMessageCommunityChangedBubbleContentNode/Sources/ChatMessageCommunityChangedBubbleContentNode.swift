...
private func communityChangedPeer(message: EngineRawMessage) -> (EnginePeer.Id, IosappCommunity)? {
    for media in message.media {
        guard let action = media as? IosappMediaAction else {
            continue
        }
        if case let .communityChanged(communityId) = action.action, let communityId, let community = message.peers[communityId] as? IosappCommunity {
