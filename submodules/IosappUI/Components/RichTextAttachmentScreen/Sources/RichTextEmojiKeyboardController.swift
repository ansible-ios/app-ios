...
/// Hosts the Iosapp emoji keyboard (`ChatEntityKeyboardInputNode`, emoji-only) as a bottom panel for
...
final class RichTextEmojiKeyboardController {
    ...
    private var emojiFiles: [Int64: IosappMediaFile] = [:]
    ...
    func seedEmojiFiles(_ files: [Int64: IosappMediaFile]) {
    ...
    /// the `IosappMediaFile` to each custom-emoji run when converting the editor's fileId-only `Document`
