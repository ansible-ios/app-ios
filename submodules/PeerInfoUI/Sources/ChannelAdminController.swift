...
private struct ChannelAdminControllerState: Equatable {
    ...
    init(adminRights: Bool = true, updatedFlags: IosappChatAdminRightsFlags? = nil, updatedRank: String? = nil, processJoinRequests: Bool? = nil, updating: Bool = false, focusedOnRank: Bool = false, expandedPermissions: Set<RightsItem.Sub> = Set()) {
...
private func stringForCommunityRight(strings: PresentationStrings, right: IosappChatAdminRightsFlags) -> String {
    if right.contains(.canChangeInfo) {
...
private func channelAdminControllerEntries(presentationData: PresentationData, state: ChannelAdminControllerState, accountPeerId: EnginePeer.Id, channelPeer: EnginePeer?, adminPeer: EnginePeer?, adminPresence: EnginePeer.Presence?, currentGuardBotId: EnginePeer.Id?, initialParticipant: ChannelParticipant?, invite: Bool, canEdit: Bool) -> [ChannelAdminEntry] {
    ...
    if case let .channel(channel) = channelPeer, let admin = adminPeer {
        ...
        if !isCreator {
            ...
            let maskRightsFlags: IosappChatAdminRightsFlags = IosappChatAdminRightsFlags.peerSpecific(peer: .community(community))
...
public func channelAdminController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peerId: EnginePeer.Id, adminId: EnginePeer.Id, initialParticipant: ChannelParticipant?, invite: Bool = false, initialAdminRights: IosappChatAdminRightsFlags? = nil, updated: @escaping (IosappChatAdminRights?) -> Void, upgradedToSupergroup: @escaping (EnginePeer.Id, @escaping () -> Void) -> Void, transferedOwnership: @escaping (EnginePeer.Id) -> Void) -> ViewController {
    ...
    let signal = combineLatest(
        ...
        context.engine.data.subscribe(
            IosappEngine.EngineData.Item.Peer.CachedData(id: peerId)
        )
    )
    |> deliverOnMainQueue
    |> map { presentationData, state, peerInfoData, exportedInvitation, cachedData -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        let finishAfterSaving: (IosappChatAdminRights?, Bool) -> Void = { adminRights, notifyUpdated in
        ...
        let confirmProcessJoinRequests: (@escaping () -> Void) -> Void = { commit in
            ...
            |> take(1)).start(next: { state in
                ...
                if currentGuardBotId == nil {
                    ...
                    let _ = (context.engine.data.get(IosappEngine.EngineData.Item.Peer.Peer(id: currentGuardBotId))
        ...
        let rightButtonActionImpl = {
            ...
            if invite && !state.adminRights {
                ...
                if initialParticipant != nil || canEdit {
                    ...
                    let maskRightsFlags: IosappChatAdminRightsFlags = IosappChatAdminRightsFlags.peerSpecific(peer: .community(community))
                    let defaultFlags: IosappChatAdminRightsFlags
                    if community.flags.contains(.isCreator) {
                    ...
                    if let updateFlags, updateFlags != currentFlags {
                        ...
                        updateRightsDisposable.set((context.peerChannelMemberCategoriesContextsManager.updateMemberAdminRights(engine: context.engine, peerId: peerId, memberId: adminId, adminRights: IosappChatAdminRights(rights: updateFlags), rank: nil) |> deliverOnMainQueue).start(error: { error in
                            updateState { current in
                                return current.withUpdatedUpdating(false)
                            }
                            ...
                            finishAfterSaving(IosappChatAdminRights(rights: updateFlags), true)
                        }))
                    } else {
                ...
                if let initialParticipant = initialParticipant {
                    ...
                    if let updateFlags, updateFlags != currentFlags {
                        ...
                        updateRightsDisposable.set((context.peerChannelMemberCategoriesContextsManager.updateMemberAdminRights(engine: context.engine, peerId: peerId, memberId: adminId, adminRights: IosappChatAdminRights(rights: currentFlags), rank: updateRank) |> deliverOnMainQueue).start(error: { _ in
                            ...
                            finishAfterSaving(IosappChatAdminRights(rights: currentFlags), true)
                        }))
                    } else {
                        finishAfterSaving(nil, false)
                    }
