...
/// decode cannot drift. Format: a markdown link `[<alt>](as://emoji?id=<fileId>)`.
public func customEmojiMarkdownURL(fileId: Int64) -> String {
    return "as://emoji?id=\(fileId)"
}
...
/// alt round-trips. Shared by every site that emits a `[<alt>](as://emoji?id=…)`
...
/// Parses the file id out of a `as://emoji?id=<fileId>` marker URL.
...
/// Regex matching an emitted marker link: `[<alt>](as://emoji?id=<digits>)`.
...
/// where each `as://emoji?id=` marker link has been turned back into a live
