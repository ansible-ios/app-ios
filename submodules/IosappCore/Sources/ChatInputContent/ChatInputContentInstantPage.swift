    ...
    while i < content.blocks.count {
        ...
        case let .media(m):
            ...
            case .image, .video, .audio:
                ...
                // image/video/audio are always a concrete IosappMediaImage/IosappMediaFile with an id; a nil-id
            ...
            case .location:
                // A location is a `IosappMediaMap` (id-less): the InstantPage `.map` block carries its coordinates
                ...
                if let map = item.media as? IosappMediaMap {
...
/// (`.last`, the convention), or a video/image file's `.Video`/`.ImageSize` attribute (`IosappMediaFile.dimensions`).
...
func chatInputBlocks(fromInstantPageBlocks blocks: [InstantPageBlock], media: [MediaId: Media] = [:]) -> [ChatInputBlock] {
    ...
    for block in blocks {
        ...
        case let .audio(id, caption):
            // Resolve the concrete `IosappMediaFile` from the page `media` dict (the forward always stores it).
        ...
        case let .map(latitude, longitude, _, _, caption):
            // Reconstruct a `IosappMediaMap` from the inline coordinates (no media-dict lookup — a `.map` block
            ...
            let map = IosappMediaMap(latitude: latitude, longitude: longitude, heading: nil, accuracyRadius: nil, venue: nil)
