...
public enum ChatInputInlineEntity: Equatable, Codable {
    ...
    case url(String)
    case date(Int32)
    case customEmoji(fileId: Int64, file: IosappMediaFile?, enableAnimation: Bool)
...
public struct ChatInputMediaItem: Equatable {
    ...
    /// Iosapp-style spoiler: the medium renders behind an animated "dust" overlay until tapped.
...
extension ChatInputMediaItem: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch mediaType {
        case .image: self.media = IosappMediaImage(decoder: mediaDecoder)
        case .file: self.media = IosappMediaFile(decoder: mediaDecoder)
        case .map: self.media = IosappMediaMap(decoder: mediaDecoder)
        }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if self.media is IosappMediaImage { mediaType = .image }
        else if self.media is IosappMediaMap { mediaType = .map }
        else if self.media is IosappMediaFile { mediaType = .file }
        else { mediaType = .file }
...
extension ChatInputMedia: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let items = try container.decodeIfPresent([ChatInputMediaItem].self, forKey: .items) {
            ...
            let media: Media
            switch mediaType {
            case .image: media = IosappMediaImage(decoder: mediaDecoder)
            case .file: media = IosappMediaFile(decoder: mediaDecoder)
            case .map: media = IosappMediaMap(decoder: mediaDecoder)
            }
