...
private func stickerSearchCombinedItems(localItems: [FoundStickerItem], files: [IosappMediaFile], isPremium: Bool, searchStickersConfiguration: SearchStickersConfiguration) -> [FoundStickerItem] {
...
private struct EmojiSearchContextInitialData {
    let localItems: [IosappMediaFile]
    let cached: CachedStickerQueryResult?
    let cacheKey: String
}

private func emojiSearchAppendUniqueItems(currentItems: [IosappMediaFile], files: [IosappMediaFile]) -> [IosappMediaFile] {
...
private func emojiSearchContextPage(account: Account, query: String, emoticon: [String], inputLanguageCode: String, scope: SearchStickersScope, offset: Int32) -> Signal<(items: [IosappMediaFile], isFinalResult: Bool, nextOffset: Int32?), NoError> {
    if offset != 0 {
        ...
        |> map { result -> (items: [IosappMediaFile], isFinalResult: Bool, nextOffset: Int32?) in
            switch result {
            case let .foundStickers(foundStickersData):
    ...
    |> mapToSignal { initialData -> Signal<(items: [IosappMediaFile], isFinalResult: Bool, nextOffset: Int32?), NoError> in
        if !scope.contains(.remote) {
            return .single((initialData.localItems, true, nil))
        }
        
        let tempResult: [IosappMediaFile]
        if let cached = initialData.cached {
        ...
        |> mapToSignal { result -> Signal<(items: [IosappMediaFile], isFinalResult: Bool, nextOffset: Int32?), NoError> in
            return account.postbox.transaction { transaction -> (items: [IosappMediaFile], isFinalResult: Bool, nextOffset: Int32?) in
                switch result {
                case let .foundStickers(foundStickersData):
                    let files = stickerSearchApiFiles(foundStickersData.stickers)
...
public final class EmojiSearchContext {
    public struct State: Equatable {
        ...
        public init(items: [IosappMediaFile], canLoadMore: Bool, isLoadingMore: Bool) {
            self.items = items
            self.canLoadMore = canLoadMore
            self.isLoadingMore = isLoadingMore
        }
    ...
    private func loadInitial() {
        ...
        let signal: Signal<(items: [IosappMediaFile], isFinalResult: Bool, nextOffset: Int32?), NoError>
        if let query = self.query, !query.isEmpty {
