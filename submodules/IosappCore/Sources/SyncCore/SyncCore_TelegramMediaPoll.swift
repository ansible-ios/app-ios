...
public struct IosappMediaPollResults: Equatable, PostboxCoding {
    ...
    public init(voters: [IosappMediaPollOptionVoters]?, totalVoters: Int32?, recentVoters: [PeerId], solution: IosappMediaPollResults.Solution?, hasUnseenVotes: Bool?, canViewStats: Bool) {
        self.voters = voters
...
public final class IosappMediaPoll: Media, Equatable {
    ...
    public init(decoder: PostboxDecoder) {
        ...
        self.results = decoder.decodeObjectForKey("rs", decoder: { IosappMediaPollResults(decoder: $0) }) as? IosappMediaPollResults ?? IosappMediaPollResults(voters: nil, totalVoters: nil, recentVoters: [], solution: nil, hasUnseenVotes: nil, canViewStats: false)
        ...
        if min {
            if let currentVoters = self.results.voters, let updatedVoters = results.voters {
                ...
                updatedResults = IosappMediaPollResults(voters: updatedVoters.map({ voters in
                    return IosappMediaPollOptionVoters(selected: selectedOpaqueIdentifiers.contains(voters.opaqueIdentifier), opaqueIdentifier: voters.opaqueIdentifier, count: voters.count, isCorrect: voters.isCorrect || correctOpaqueIdentifiers.contains(voters.opaqueIdentifier), recentVoters: voters.recentVoters)
                ...
                updatedResults = IosappMediaPollResults(voters: updatedVoters, totalVoters: results.totalVoters, recentVoters: results.recentVoters, solution: results.solution ?? self.results.solution, hasUnseenVotes: results.hasUnseenVotes ?? self.results.hasUnseenVotes, canViewStats: results.canViewStats)
            } else {
                updatedResults = IosappMediaPollResults(voters: self.results.voters, totalVoters: results.totalVoters, recentVoters: results.recentVoters, solution: results.solution ?? self.results.solution, hasUnseenVotes: results.hasUnseenVotes ?? self.results.hasUnseenVotes, canViewStats: results.canViewStats)
            }
        } else {
    ...
    public func withoutUnreadResults() -> IosappMediaPoll {
        let updatedResults = IosappMediaPollResults(voters: self.results.voters, totalVoters: self.results.totalVoters, recentVoters: self.results.recentVoters, solution: self.results.solution, hasUnseenVotes: false, canViewStats: self.results.canViewStats)
        return IosappMediaPoll(pollId: self.pollId, publicity: self.publicity, kind: self.kind, text: self.text, textEntities: self.textEntities, options: self.options, correctAnswers: self.correctAnswers, results: updatedResults, isClosed: self.isClosed, deadlineTimeout: self.deadlineTimeout, deadlineDate: self.deadlineDate, pollHash: self.pollHash, openAnswers: self.openAnswers, revotingDisabled: self.revotingDisabled, shuffleAnswers: self.shuffleAnswers, hideResultsUntilClose: self.hideResultsUntilClose, isCreator: self.isCreator, attachedMedia: self.attachedMedia, restrictToSubscribers: self.restrictToSubscribers, countries: self.countries)
    }
}
