...
extension RichText {
    public init(flatBuffersObject: IosappCore_RichText) throws {
        ...
        case .richtextFormula:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_Formula.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .formula(latex: value.latex)
        case .richtextCustomemoji:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_CustomEmoji.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textCustomEmoji(fileId: value.fileId, alt: value.alt)
        case .richtextAutoemail:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_AutoEmail.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textAutoEmail(text: try RichText(flatBuffersObject: value.text))
        case .richtextAutophone:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_AutoPhone.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textAutoPhone(text: try RichText(flatBuffersObject: value.text))
        case .richtextAutourl:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_AutoUrl.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textAutoUrl(text: try RichText(flatBuffersObject: value.text))
        case .richtextBankcard:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_BankCard.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textBankCard(text: try RichText(flatBuffersObject: value.text))
        case .richtextBotcommand:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_BotCommand.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textBotCommand(text: try RichText(flatBuffersObject: value.text))
        case .richtextCashtag:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_Cashtag.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textCashtag(text: try RichText(flatBuffersObject: value.text))
        case .richtextHashtag:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_Hashtag.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textHashtag(text: try RichText(flatBuffersObject: value.text))
        case .richtextMention:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_Mention.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textMention(text: try RichText(flatBuffersObject: value.text))
        case .richtextMentionname:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_MentionName.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textMentionName(text: try RichText(flatBuffersObject: value.text), peerId: value.peerId)
        case .richtextSpoiler:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_Spoiler.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .textSpoiler(text: try RichText(flatBuffersObject: value.text))
        case .richtextDate:
            guard let value = flatBuffersObject.value(type: IosappCore_RichText_Date.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
    ...
    public func encodeToFlatBuffers(builder: inout FlatBufferBuilder) -> Offset {
        ...
        case let .formula(latex):
            ...
            let start = IosappCore_RichText_Formula.startRichText_Formula(&builder)
            IosappCore_RichText_Formula.add(latex: latexOffset, &builder)
            offset = IosappCore_RichText_Formula.endRichText_Formula(&builder, start: start)
        case let .textCustomEmoji(fileId, alt):
            valueType = .richtextCustomemoji
            let altOffset = builder.create(string: alt)
            let start = IosappCore_RichText_CustomEmoji.startRichText_CustomEmoji(&builder)
            IosappCore_RichText_CustomEmoji.add(fileId: fileId, &builder)
            IosappCore_RichText_CustomEmoji.add(alt: altOffset, &builder)
            offset = IosappCore_RichText_CustomEmoji.endRichText_CustomEmoji(&builder, start: start)
        case let .textAutoEmail(text):
            valueType = .richtextAutoemail
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_AutoEmail.startRichText_AutoEmail(&builder)
            IosappCore_RichText_AutoEmail.add(text: textOffset, &builder)
            offset = IosappCore_RichText_AutoEmail.endRichText_AutoEmail(&builder, start: start)
        case let .textAutoPhone(text):
            valueType = .richtextAutophone
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_AutoPhone.startRichText_AutoPhone(&builder)
            IosappCore_RichText_AutoPhone.add(text: textOffset, &builder)
            offset = IosappCore_RichText_AutoPhone.endRichText_AutoPhone(&builder, start: start)
        case let .textAutoUrl(text):
            valueType = .richtextAutourl
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_AutoUrl.startRichText_AutoUrl(&builder)
            IosappCore_RichText_AutoUrl.add(text: textOffset, &builder)
            offset = IosappCore_RichText_AutoUrl.endRichText_AutoUrl(&builder, start: start)
        case let .textBankCard(text):
            valueType = .richtextBankcard
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_BankCard.startRichText_BankCard(&builder)
            IosappCore_RichText_BankCard.add(text: textOffset, &builder)
            offset = IosappCore_RichText_BankCard.endRichText_BankCard(&builder, start: start)
        case let .textBotCommand(text):
            valueType = .richtextBotcommand
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_BotCommand.startRichText_BotCommand(&builder)
            IosappCore_RichText_BotCommand.add(text: textOffset, &builder)
            offset = IosappCore_RichText_BotCommand.endRichText_BotCommand(&builder, start: start)
        case let .textCashtag(text):
            valueType = .richtextCashtag
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_Cashtag.startRichText_Cashtag(&builder)
            IosappCore_RichText_Cashtag.add(text: textOffset, &builder)
            offset = IosappCore_RichText_Cashtag.endRichText_Cashtag(&builder, start: start)
        case let .textHashtag(text):
            valueType = .richtextHashtag
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_Hashtag.startRichText_Hashtag(&builder)
            IosappCore_RichText_Hashtag.add(text: textOffset, &builder)
            offset = IosappCore_RichText_Hashtag.endRichText_Hashtag(&builder, start: start)
        case let .textMention(text):
            valueType = .richtextMention
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_Mention.startRichText_Mention(&builder)
            IosappCore_RichText_Mention.add(text: textOffset, &builder)
            offset = IosappCore_RichText_Mention.endRichText_Mention(&builder, start: start)
        case let .textMentionName(text, peerId):
            valueType = .richtextMentionname
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_MentionName.startRichText_MentionName(&builder)
            IosappCore_RichText_MentionName.add(text: textOffset, &builder)
            IosappCore_RichText_MentionName.add(peerId: peerId, &builder)
            offset = IosappCore_RichText_MentionName.endRichText_MentionName(&builder, start: start)
        case let .textSpoiler(text):
            valueType = .richtextSpoiler
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_Spoiler.startRichText_Spoiler(&builder)
            IosappCore_RichText_Spoiler.add(text: textOffset, &builder)
            offset = IosappCore_RichText_Spoiler.endRichText_Spoiler(&builder, start: start)
        case let .textDate(text, date, format):
            valueType = .richtextDate
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_RichText_Date.startRichText_Date(&builder)
            IosappCore_RichText_Date.add(text: textOffset, &builder)
            IosappCore_RichText_Date.add(date: date, &builder)
            IosappCore_RichText_Date.add(format: format?.rawValue ?? -1, &builder)
            offset = IosappCore_RichText_Date.endRichText_Date(&builder, start: start)
        }
