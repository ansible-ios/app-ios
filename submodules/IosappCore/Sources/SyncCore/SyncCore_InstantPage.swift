...
public indirect enum InstantPageBlock: PostboxCoding, Equatable {
    ...
    public init(flatBuffersObject: IosappCore_InstantPageBlock) throws {
        ...
        case .instantpageblockHeading:
            guard let value = flatBuffersObject.value(type: IosappCore_InstantPageBlock_Heading.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
            self = .heading(text: try RichText(flatBuffersObject: value.text), level: value.level)
        case .instantpageblockFormula:
            guard let value = flatBuffersObject.value(type: IosappCore_InstantPageBlock_Formula.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
        ...
        case .instantpageblockThinking:
            guard let value = flatBuffersObject.value(type: IosappCore_InstantPageBlock_Thinking.self) else {
                throw FlatBuffersError.missingRequiredField()
            }
    ...
    public func encodeToFlatBuffers(builder: inout FlatBufferBuilder) -> Offset {
        ...
        case let .heading(text, level):
            ...
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_InstantPageBlock_Heading.startInstantPageBlock_Heading(&builder)
            IosappCore_InstantPageBlock_Heading.add(text: textOffset, &builder)
            IosappCore_InstantPageBlock_Heading.add(level: level, &builder)
            offset = IosappCore_InstantPageBlock_Heading.endInstantPageBlock_Heading(&builder, start: start)
        case let .formula(latex):
            valueType = .instantpageblockFormula
            let latexOffset = builder.create(string: latex)
            let start = IosappCore_InstantPageBlock_Formula.startInstantPageBlock_Formula(&builder)
            IosappCore_InstantPageBlock_Formula.add(latex: latexOffset, &builder)
            offset = IosappCore_InstantPageBlock_Formula.endInstantPageBlock_Formula(&builder, start: start)
        case let .paragraph(text):
        ...
        case let .preformatted(text, language):
            ...
            if let languageOffset {
                IosappCore_InstantPageBlock_Preformatted.add(language: languageOffset, &builder)
            }
        ...
        case let .blockQuote(blocks, caption, _):
            ...
            IosappCore_InstantPageBlock_BlockQuote.addVectorOf(blocks: blocksOffset, &builder)
        ...
        case let .image(id, caption, url, webpageId, spoiler):
            ...
            IosappCore_InstantPageBlock_Image.add(spoiler: spoiler, &builder)
        ...
        case let .video(id, caption, autoplay, loop, spoiler):
            ...
            IosappCore_InstantPageBlock_Video.add(spoiler: spoiler, &builder)
        ...
        case let .thinking(text):
            ...
            let textOffset = text.encodeToFlatBuffers(builder: &builder)
            let start = IosappCore_InstantPageBlock_Thinking.startInstantPageBlock_Thinking(&builder)
            IosappCore_InstantPageBlock_Thinking.add(text: textOffset, &builder)
            offset = IosappCore_InstantPageBlock_Thinking.endInstantPageBlock_Thinking(&builder, start: start)
        case let .table(title, rows, bordered, striped):
...
public indirect enum InstantPageListItem: PostboxCoding, Equatable {
    ...
    public func encodeToFlatBuffers(builder: inout FlatBufferBuilder) -> Offset {
        ...
        case let .text(text, number, checked):
            ...
            if let triState = InstantPageListItem.triState(fromChecked: checked) {
                IosappCore_InstantPageListItem_Text.add(checkState: triState, &builder)
            }
        ...
        case let .blocks(blocks, number, checked):
            ...
            if let triState = InstantPageListItem.triState(fromChecked: checked) {
                IosappCore_InstantPageListItem_Blocks.add(checkState: triState, &builder)
            }
