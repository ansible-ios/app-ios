...
private func shouldUseStoredPeerChatInfoForPeerWithoutValidInclusion(transaction: Transaction, peerId: PeerId, peer: Peer) -> Bool {
    ...
    if let community = peer as? IosappCommunity, community.participationStatus == .member {
        ...
        case let .updatePinnedForumTopics(updatePinnedForumTopicsData):
                ...
                if popup {
                    ...
                    if updatedState.peers[peerId] == nil {
                        updatedState.updatePeer(peerId, { peer in
                            if peer == nil {
                                return IosappUser(id: peerId, accessHash: nil, firstName: "Iosapp Notifications", lastName: nil, username: nil, phone: nil, photo: [], botInfo: BotUserInfo(flags: [], inlinePlaceholder: nil), restrictionInfo: nil, flags: [.isVerified], emojiStatus: nil, usernames: [], storiesHidden: nil, nameColor: nil, backgroundEmojiId: nil, profileColor: nil, profileBackgroundEmojiId: nil, subscriberCount: nil, verificationIconFileId: nil)
                            } else {
                                return peer
                            }
                        })
            ...
            case let .updateChatDefaultBannedRights(updateChatDefaultBannedRightsData):
                ...
                updatedState.updatePeer(peer.peerId, { peer in
                    ...
                    } else if let community = peer as? IosappCommunity {
                        return community.withUpdatedDefaultBannedRights(IosappChatBannedRights(apiBannedRights: defaultBannedRights))
                    } else {
                        return peer
                    }
                })
...
func extractEmojiFileIds(message: StoreMessage, fileIds: inout Set<Int64>) {
    ...
    for media in message.media {
        if let media = media as? IosappMediaWebpage, case let .Loaded(content) = media.content {
                    ...
                    for dialog in dialogs {
                        switch dialog {
                            ...
                            case let .dialogCommunity(dialogCommunityData):
                                ...
                                let notificationSettings = IosappPeerNotificationSettings(apiSettings: dialogCommunityData.notifySettings)
                ...
                case let .peerDialogs(peerDialogsData):
                    ...
                    loop: for dialog in dialogs {
                        ...
                        switch dialog {
                            ...
                            case let .dialogCommunity(dialogCommunityData):
                                let peerId = peerIdFromApiCommunityId(dialogCommunityData.communityId)
                                notificationSettings[peerId] = IosappPeerNotificationSettings(apiSettings: dialogCommunityData.notifySettings)
                                continue loop
                            case .dialogFolder:
                                assertionFailure()
                                continue loop
