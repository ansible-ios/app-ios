...
func isPeerHiddenByCollapsedCommunity(transaction: Transaction, peerId: PeerId, peer: Peer? = nil) -> Bool {
    if let channel = (peer ?? transaction.getPeer(peerId)) as? IosappChannel, let linkedCommunityId = channel.linkedCommunityId {
        if let community = transaction.getPeer(linkedCommunityId) as? IosappCommunity, community.collapsedInDialogs == true {
            return true
        }
    }
...
public func updatePeersCustom(transaction: Transaction, peers: [Peer], update: (Peer?, Peer) -> Peer?) {
    transaction.updatePeersInternal(peers, update: { previous, updated in
        ...
        if let updatedCommunity = updated as? IosappCommunity {
            updated = mergeCommunity(lhs: previous as? IosappCommunity, rhs: updatedCommunity)
        }
        ...
        switch peerId.namespace {
            ...
            case Namespaces.Peer.CloudChannel:
                ...
                } else if let community = updated as? IosappCommunity {
...
func updatePeerPresencesClean(transaction: Transaction, accountPeerId: PeerId, peerPresences: [PeerId: UpdatedApiPresence]) {
    ...
    for (peerId, status) in peerPresences {
        let presence = IosappUserPresence(apiStatus: status.status)
        switch presence.status {
        case .present:
            parsedPresences[peerId] = presence
        default:
