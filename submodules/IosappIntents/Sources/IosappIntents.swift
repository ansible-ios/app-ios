    ...
    if #available(iOSApplicationExtension 13.2, iOS 13.2, *) {
        ...
        |> deliverOnMainQueue).start(next: { peers in
            ...
            for (peer, _, avatarImage) in peers {
                ...
                let recipient = INPerson(personHandle: recipientHandle, nameComponents: nameComponents, displayName: displayTitle, image: personImage, contactIdentifier: nil, customIdentifier: "as\(peer.id.toInt64())")
               
                let intent = INSendMessageIntent(recipients: [recipient], content: nil, speakableGroupName: INSpeakableString(spokenPhrase: displayTitle), conversationIdentifier: "as\(peer.id.toInt64())", serviceName: nil, sender: nil)
