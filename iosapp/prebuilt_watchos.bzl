...
The watch source tree is the committed in-repo snapshot at `Iosapp/WatchApp/` (tracked
...
def _apple_prebuilt_watchos_application_impl(ctx):
    # The watch app is built from the committed in-repo snapshot at Iosapp/WatchApp,
...
apple_prebuilt_watchos_application = rule(
    ...
    attrs = {
        ...
        "_worker": attr.label(
            default = "//Iosapp:prebuilt_watchos_build.sh",
            allow_single_file = True,
        ),
    },
)
