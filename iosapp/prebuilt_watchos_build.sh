...
#                   (Iosapp/WatchApp), which contains tgwatch.xcodeproj.
...
if [ ! -e "$SRC/tgwatch.xcodeproj" ]; then
  echo "error: no tgwatch.xcodeproj at $SRC (re-sync the Iosapp/WatchApp snapshot via tgwatch/tools/export-sources.sh)" >&2
  exit 1
fi
