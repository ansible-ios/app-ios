...
/// Bands the seconds value into a human label matching Iosapp desktop.
...
func serviceLineText(
    ...
    case .messageContactRegistered:
        return withActor("joined Iosapp", actor: actor, includeActor: includeActor)
    ...
    case .messageVideoChatEnded(let m):
        // Actor intentionally dropped — TDLib emits one but stock Iosapp clients
    ...
    case .messageBasicGroupChatCreate:
        // Chat-lifecycle event — rendered without an actor (Iosapp convention).
    ...
    case .messageChatUpgradeTo, .messageChatUpgradeFrom:
        // Chat-lifecycle event — actor-less (Iosapp convention), ignores includeActor.
