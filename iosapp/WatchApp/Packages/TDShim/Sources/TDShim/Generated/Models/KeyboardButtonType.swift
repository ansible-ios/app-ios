...
public struct KeyboardButtonTypeRequestUsers: Codable, Equatable, Hashable, Identifiable {
    ...
    /// True, if the shared users must or must not be Iosapp Premium users
    ...
    /// True, if the shared users must be Iosapp Premium users; otherwise, the shared users must not be Iosapp Premium users. Ignored if restrict_user_is_premium is false
