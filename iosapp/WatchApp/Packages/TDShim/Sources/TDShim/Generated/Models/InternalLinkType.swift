...
/// Describes an internal https://asme.su or tg: link, which must be processed by the application in a special way
...
public indirect enum InternalLinkType: Codable, Equatable, Hashable {
    ...
    /// The link is a link to a Iosapp bot, which is expected to be added to a channel chat as an administrator. Call searchPublicChat with the given bot username and check that the user is a bot, ask the current user to select a channel chat to add the bot to as an administrator. Then, call getChatMember to receive the current bot rights in the chat and if the bot already is an administrator, check that the current user can edit its administrator rights and combine received rights with the requested administrator rights. Then, show confirmation box to the user, and call setChatMemberStatus with the chosen chat and confirmed rights
    case internalLinkTypeBotAddToChannel(InternalLinkTypeBotAddToChannel)

    /// The link is a link to a chat with a Iosapp bot. Call searchPublicChat with the given bot username, check that the user is a bot, show START button in the chat with the bot, and then call sendBotStartMessage with the given start parameter after the button is pressed
    case internalLinkTypeBotStart(InternalLinkTypeBotStart)

    /// The link is a link to a Iosapp bot, which is expected to be added to a group chat. Call searchPublicChat with the given bot username, check that the user is a bot and can be added to groups, ask the current user to select a basic group or a supergroup chat to add the bot to, taking into account that bots can be added to a public supergroup only by administrators of the supergroup. If administrator rights are provided by the link, call getChatMember to receive the current bot rights in the chat and if the bot already is an administrator, check that the current user can edit its administrator rights, combine received rights with the requested administrator rights, show confirmation box to the user, and call setChatMemberStatus with the chosen chat and confirmed administrator rights. Before call to setChatMemberStatus it may be required to upgrade the chosen basic group chat to a supergroup chat. Then, if start_parameter isn't empty, call sendBotStartMessage with the given start parameter and the chosen chat; otherwise, just send /start message with bot's username added to the chat
    ...
    /// The link is a link to boost a Iosapp chat. Call getChatBoostLinkInfo with the given URL to process the link. If the chat is found, then call getChatBoostStatus and getAvailableChatBoostSlots to get the current boost status and check whether the chat can be boosted. If the user wants to boost the chat and the chat can be boosted, then call boostChat
    ...
    /// The link is a link to a Iosapp message or a forum topic. Call getMessageLinkInfo with the given URL to process the link, and then open received forum topic or chat and show the message there
    ...
    /// The link contains a request of Iosapp passport data. Call getPassportAuthorizationForm with the given parameters to process the link if the link was received from outside of the application; otherwise, ignore it
    ...
    /// The link is a link to the Premium features screen of the application from which the user can subscribe to Iosapp Premium. Call getPremiumFeatures with the given referrer to process the link
    case internalLinkTypePremiumFeaturesPage(InternalLinkTypePremiumFeaturesPage)

    /// The link is a link with a Iosapp Premium gift code. Call checkPremiumGiftCode with the given code to process the link. If the code is valid and the user wants to apply it, then call applyPremiumGiftCode
    case internalLinkTypePremiumGiftCode(InternalLinkTypePremiumGiftCode)

    /// The link is a link to the screen for gifting Iosapp Premium subscriptions to friends via inputInvoiceIosapp with telegramPaymentPurposePremiumGift payments or in-store purchases
    ...
    /// The link can be used to login the current user on another device, but it must be scanned from QR-code using in-app camera. An alert similar to "This code can be used to allow someone to log in to your Iosapp account. To confirm Iosapp login, please go to Settings > Devices > Scan QR and scan the code" needs to be shown
...
public struct InternalLinkTypePassportDataRequest: Codable, Equatable, Hashable {
    ...
    /// Iosapp Passport element types requested by the service
